...
\subsection{Data Collection and Preprocessing}
To ensure the empirical validity and robustness of the model, we moved beyond basic datasets and constructed a comprehensive database covering the period from 2010 to 2025. Our data collection focused on seven core dimensions, ranging from macroeconomic indicators to micro-level brand performance, to accurately capture the dynamics of U.S.-Japan auto trade.

\subsubsection{Data Sources and Indicator Selection}
We systematically collected data to quantify market behavior, supply chain costs, and policy impacts. The detailed classification of our self-collected database is presented in Table \ref{tab:q2_data_sources}.

\begin{table}[H]
\centering
\caption{Comprehensive Data Indicators and Classification (2010--2025)}
\begin{tabular}{l>{\raggedright\arraybackslash}p{4cm}>{\raggedright\arraybackslash}p{7.5cm}}
\toprule
\textbf{Category} & \textbf{Sub-Category} & \textbf{Core Indicators Collected} \\
\midrule
\multirow{2}{*}{\textbf{Market Data}} & Total Market & Total U.S. Auto Sales (2010--2025), Import Share (Japan vs. Mexico origin) \\
 & Brand Performance & Sales volume of Toyota, Honda, Nissan; Aggregate Japanese market share; Top 10 best-selling models \\
\midrule
\multirow{2}{*}{\textbf{Trade Data}} & Export Flows & Direct exports (Japan $\to$ U.S.); Trans-shipments (Mexico/Canada $\to$ U.S.) \\
 & Policy & MFN Tariffs, Additional Tariffs, USMCA Rules of Origin compliance \\
\midrule
\multirow{2}{*}{\textbf{Industrial}} & Production & Local U.S. production by Japanese firms, Number of factories, Capacity utilization \\
 & Socio-Economic & Auto industry employment figures (political leverage proxy) \\
\midrule
\multirow{2}{*}{\textbf{Macro \& Tech}} & Economic & USD/JPY Exchange Rates (2010--2025), U.S. Gasoline Prices \\
 & Technology & HEV/EV market share, Reliability scores, Resale value retention \\
\midrule
\textbf{Policy} & Environment & IRA EV Subsidies availability, Fuel efficiency standards \\
\bottomrule
\end{tabular}
\label{tab:q2_data_sources}
\end{table}

\subsubsection{Descriptive Statistical Analysis}

\paragraph{1. Tariff Policy and Trade Barriers (2010--2025)}
Our data tracks the evolution of trade barriers. From 2010 to 2024, the baseline MFN tariff remained at 2.5\% for passenger cars and 25\% for light trucks. However, the implementation of the USMCA (replacing NAFTA) tightened Rules of Origin. For the 2025 scenario, we incorporate the proposed "Reciprocal Tariffs" impacting both direct imports and non-compliant North American transfers.

\paragraph{2. Market Structure: The Shift from Import to Localization}
Analyzing the "Japanese Brand Data" and "Industrial Data," we observe a structural shift. While total U.S. sales fluctuated (peaking at 17.55M in 2016), direct imports from Japan have stabilized.
\begin{itemize}
    \item \textbf{Local Production:} Major firms (Toyota, Honda) have significantly increased U.S. production capacity to mitigate exchange rate risks and tariff barriers.
    \item \textbf{Mexico Leakage:} A substantial portion of "Japanese brand" sales now originates from assembly plants in Mexico, leveraging the USMCA duty-free access (prior to new tariff enforcements).
\end{itemize}

\paragraph{3. Cost Drivers: Exchange Rates and Energy}
\begin{itemize}
    \item \textbf{USD/JPY Exchange Rate:} The Yen depreciated significantly from $\approx 80$ (2012) to $\approx 151$ (2024), theoretically lowering the cost of direct imports ($c_J$).
    \item \textbf{Consumer Preference:} High gasoline prices and reliability scores have sustained demand for Japanese HEVs (Hybrids), creating a specific stickiness in demand ($Q_{base}$) despite potential price hikes.
\end{itemize}

\subsection{Model Construction}

\subsubsection{Supply Side: Three-Path Cost Game}
Based on the trade routes identified in our data, we model the cost structure for a representative vehicle across three supply paths:

\textbf{Path J (Direct Import from Japan):} Subject to exchange rates and new tariffs.
\[
C_J = \left(\frac{c_{JPY}}{R_{ex}} + t_{sea}\right) (1 + \tau_{base} + \tau_{add})
\]

\textbf{Path M (Transplant via Mexico):} Subject to USMCA compliance and land freight.
\[
C_M = (c_M + t_{land}) (1 + \tau_{USMCA})
\]

\textbf{Path U (U.S. Local Production):} Subject to higher labor costs but zero tariffs and potential IRA subsidies.
\[
C_U = c_U + F_{amort} - S_{IRA}
\]

\subsubsection{Market Share Allocation (Logit Model)}
\[
S_i = \frac{\exp(-\lambda C_i)}{\sum_{k\in\{J,M,U\}} \exp(-\lambda C_k)}
\]

\subsubsection{Demand Side: Economic Transmission Effects}
\[
Q_{new} = Q_{base}\left[ 1 + \varepsilon \cdot \left(\frac{P_{avg}-P_{base}}{P_{base}}\right) + \beta \cdot \text{Reliability} \right]
\]

\subsection{Model Solving and Parameter Calibration}

Parameters were calibrated using the collected 2010--2025 historical datasets (Table \ref{tab:q2_param_calib}).

\begin{table}[H]
\centering
\caption{Key Parameter Calibration based on Collected Data}
\begin{tabular}{lcc>{\raggedright\arraybackslash}p{5cm}}
\toprule
\textbf{Parameter} & \textbf{Symbol} & \textbf{Value} & \textbf{Data Source / Basis} \\
\midrule
Avg Exchange Rate (2024) & $R_{ex}$ & 151.46 & Historical Macro Data (USD/JPY) \\
Sea Freight & $t_{sea}$ & 1600 & Logistics Industry avg \\
Land Freight & $t_{land}$ & 600 & North American logistics data \\
U.S. Production Cost & $c_U$ & 24,000 & Derived from Industrial Data \\
IRA Subsidy Effect & $S_{IRA}$ & 0--7500 & Policy Data (Model dependent) \\
Demand Elasticity & $\varepsilon$ & -3.0 & Calibrated from historical sales vs. price \\
Baseline Sales & $Q_{base}$ & 4.15M & Aggregated Japanese brand sales \\
Tariff (New Policy) & $\tau_{add}$ & 0--50\% & Simulation Variable \\
\bottomrule
\end{tabular}
\label{tab:q2_param_calib}
\end{table}

\subsubsection{Key Scenario Solving Results}

\begin{table}[H]
\centering
\caption{Scenario Simulation Results: Cost \& Allocation}
\begin{tabular}{lccccc}
\toprule
\textbf{Scenario} & $\tau_{add}$ & Tariff (J/M) & $C_J$ (Direct) & $C_M$ (Mexico) & $C_U$ (US Local) \\
\midrule
Baseline (Status Quo) & 0\% & 2.5 / 0 & 24,618 & 23,322 & 24,900 \\
Policy Shock A & 10\% & 12.5 / 0 & 26,758 & 23,322 & 24,900 \\
Policy Shock B & 25\% & 27.5 / 25 & 31,040 & 29,150 & 24,900 \\
\bottomrule
\end{tabular}
\label{tab:q2_scenario_results}
\end{table}

\subsection{Result Analysis}

\subsubsection{Impact on U.S.-Japan Auto Trade Dynamics}
\begin{itemize}
    \item \textbf{Threshold Effect:} When $\tau_{add}$ exceeds 10\%, the exchange rate advantage of the Yen is completely nullified. Direct exports from Japan drop sharply.
    \item \textbf{Investment Diversion:} Japanese firms are forced to accelerate "Reshoring" to the U.S. (Path U) rather than exporting, validating the political goal of the tariffs.
\end{itemize}

\subsubsection{Supply Chain Reconfiguration (The "Mexico Problem")}
Our model reveals a critical insight regarding the "Mexico Leakage" effect. In scenarios where tariffs are applied only to Japan (and not Mexico), trade volume simply shifts from Path J to Path M. However, if the administration revokes USMCA preferences (Scenario B), costs for both paths skyrocket, leading to a significant contraction in total market demand due to price pass-through.

\subsubsection{Impact on the U.S. Auto Industry}
While higher tariffs protect local manufacturing ($C_U$ becomes relatively cheaper), the consumer welfare loss is significant. The average vehicle price ($P_{avg}$) rises, and our demand function indicates a potential sales drop of $\approx 450,000$ units in the extreme scenario, threatening dealership employment.
