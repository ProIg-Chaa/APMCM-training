...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/ques2/supply_chain_reconfiguration.jpg}
    \caption{Simulated Supply Chain Shifts under Varying $\tau_{add}$}
    \label{fig:supply_chain_reconfig}
\end{figure}
...
\end{equation}
Setting the "Alternative Country" (e.g., Vietnam) as the benchmark ($C_{Alt} = 100$) and cost sensitivity $\lambda = 0.15$, we obtained the following calibrated costs (Table \ref{tab:cost_calibration}):
...
\begin{itemize}
    ...
    \item $I_{RnD}$: The Innovation Factor. If Export Controls are strict but no subsidies are provided, US firms lose revenue from China, causing $I_{RnD}$ to drop to $0.7$ (R\&D Budget Cuts). If subsidies exist, $I_{RnD} = 1.0$.
\end{itemize}
...
The simulation results (Table \ref{tab:model_results}) reveal significant structural differences across chip tiers.
