...
\subsubsection{Data Sources and Indicator Selection}
All data were extracted or derived from the provided datasets, with no external data introduced. The key data sources and corresponding indicators are summarized in Table \ref{tab:q2_data_sources}.

\begin{table}[H]
\centering
\caption{Data Sources and Core Indicators for Problem 2}
\begin{tabular}{lcc>{\raggedright\arraybackslash}p{6cm}}
\toprule
\textbf{Data Category} & \textbf{Data Source File} & \textbf{Time Range} & \textbf{Core Indicators} \\
\midrule
...
\paragraph{1. U.S. Auto Tariff Trend (2010--2025)}
The MFN car tariff rate remained stable at 2.5\% from 2010 to 2024, but jumped to 15\% in 2025 under the "Reciprocal Tariffs" policy. Detailed tariff data are shown in Table \ref{tab:q2_tariff_data}.

\begin{table}[H]
\centering
\caption{U.S. MFN Auto Tariff Rates (2010--2025)}
\begin{tabular}{lcc}
\toprule
\textbf{Year} & \textbf{MFN Car Tariff (\%)} & \textbf{MFN Light Truck Tariff (\%)} \\
\midrule
2010 & 2.5 & 25 \\
2011 & 2.5 & 25 \\
2012 & 2.5 & 25 \\
...
\bottomrule
\end{tabular}
\label{tab:q2_tariff_data}
\end{table}

\paragraph{2. U.S. Auto Market Demand and Japan Import Structure (2015--2025)}

The U.S. auto market experienced fluctuations from 2015 to 2025: total sales peaked at 17.55 million in 2016, dropped to 14.50 million in 2020 due to the pandemic, and gradually recovered to 15.98 million in 2024. Japan maintained an import share of 8.5\%--10.8\% during this period. Detailed data are shown in Table \ref{tab:q2_sales_import_data}.

\begin{table}[H]
\centering
\caption{U.S. Auto Sales and Imports from Japan (2015--2025)}
\begin{tabular}{lcccc}
\toprule
\textbf{Year} & \textbf{Total Sales (million)} & \textbf{Imports from Japan (million)} & \textbf{Import Share (\%)} & \textbf{Remarks} \\
\midrule
2015 & 17.47 & 1.60 & 9.2 & Market peak, stable imports \\
2016 & 17.55 & 1.62 & 9.2 & Historical sales peak \\
2017 & 17.13 & 1.65 & 9.6 & Passenger car demand down, SUV up \\
2018 & 17.21 & 1.63 & 9.5 & -- \\
2019 & 16.96 & 1.58 & 9.3 & Before pandemic \\
2020 & 14.50 & 1.26 & 8.7 & Supply chain shock \\
2021 & 14.93 & 1.35 & 9.0 & Chip shortage \\
2022 & 13.75 & 1.49 & 10.8 & Local production drop \\
2023 & 15.62 & 1.49 & 9.5 & Market recovery \\
2024 & 15.98 & 1.52 & 9.5 & US-based Japanese production 3.2M+ \\
2025* & 15.80 & 1.35 & 8.5 & Tariff impact \\
\bottomrule
\end{tabular}
\label{tab:q2_sales_import_data}
\end{table}

\paragraph{3. Exchange Rate and Production Cost Calibration}

- USD/JPY depreciated from 121.05 (2015) to 151.46 (2024).  
- Japanese manufacturing cost:  
\[
c_J = \frac{3{,}000{,}000}{151.46} \approx 19{,}807 \text{ USD}
\]
- Mexican cost: 19,680 USD  
- U.S. cost: 24,000 USD  

\subsection{Model Construction}

\subsubsection{Supply Side: Three-Path Cost Game (Non-Tariff Responses)}
...
\begin{table}[H]
\centering
\caption{Key Parameter Calibration for Problem 2}
\begin{tabular}{lcc>{\raggedright\arraybackslash}p{5cm}}
\toprule
\textbf{Parameter} & \textbf{Symbol} & \textbf{Value} & \textbf{Basis} \\
\midrule
...
Cost sensitivity & $\lambda$ & 0.2 & Market calibration \\
Price elasticity & $\varepsilon$ & -3.0 & Literature \\
Baseline sales & $Q_{base}$ & 4.15M & From topmodel.csv \\
Benchmark tariff & $\tau_{base}$ & 15\% & CSV data \\
Additional tariff & $\tau_{add}$ & 0--50\% & Scenario simulation \\
\bottomrule
\end{tabular}
...
\begin{table}[H]
\centering
\caption{Key Scenario Simulation Results for Problem 2}
\begin{tabular}{lccccc}
\toprule
\textbf{Scenario} & $\tau_{add}$ & Total Tariff (J/M) & $C_J$ & $C_M$ & $C_U$ \\
\midrule
...
\subsubsection{Impact on U.S.-Japan Auto Trade}
- Direct exports fall below 1.2M units when $\tau_{add}>5\%$.
- Tariffs offset yen depreciation effects; Japanese firms shift to U.S. FDI.

\subsubsection{Impact on U.S. Auto Import Structure}

% Insert Fig 6.4.2: Import Structure Shift (Source & Content)

1. Source substitution: Japan ↓, Mexico ↑ (trade diversion).  
2. Content substitution: parts imports ↑ to 20B USD.

\subsubsection{Impact on U.S. Auto Industry}

% Insert Fig 6.4.3: US Auto Industry (Reshoring vs Demand)

- Reshoring: U.S. production rises from 2.6M → 3.0M.  
- Demand contraction: higher prices reduce sales by ~450k vehicles.

\subsubsection{Deep Insight: Mexico Leakage Effect}
